...
'''
perm_dict = dict()
with open('correct_t.max') as i:
    perm_dict['correct'] = [float(l.strip()) for l in i.readlines()]
with open('wrong_t.max') as i:
    perm_dict['wrong'] = [float(l.strip()) for l in i.readlines()]
...
for condition, condition_dict in true_data.items():
    ...
    for time_point, time_dict in condition_dict.items():
        for elec_code, elec_list in time_dict.items():
            ...
            if p_value/2 <= .4:
                print([condition, p_value, elec_code, time_point])
